 ...
 * in each of the three self-contained HTML files — no build step, no dependencies,
 * just Node's built-in vm/fs/assert. Run with: node tests/moveable-feasts.test.js
 */
const fs = require('fs');
...
const FILES = ['index.html', 'dual.html', 'polished.html'];
...
// '{'/'}' inside a Norwegian description string can't desync the count.
...
for (const file of FILES) {
  ...
  test('buildMoveables() places every known feast on the correct weekday, for 2000-2100', () => {
    for (const yr of YEARS) {
      ...
      for (const [name, expectedWd] of Object.entries(EXPECTED_WEEKDAY)) {
...
test('easter() agrees across all three HTML files, for 2000-2100', () => {
...
test('buildMoveables() has the same set of feast names across all three HTML files', () => {
  const loaded = FILES.map(loadFns);
