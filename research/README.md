...
|---|---|---|
| `thin` | 360 + 16 moveable | entry exists (either full `fixed`/`moveable` content, or a name-only `nameday`) but has zero sources on record, so it's `proposed` under the new standard, not `confirmed` |
...
16 `moveable` entries have real, usable content, it just predates the
